...
```text
<neuron-id>/
  agent/          # go.mod, main.go, internal/skills/
...
1. **Agent module** — `go.mod` under `~/alis.build/<lz>/build/<product>/<neuron>/agent/`. Runtime skills live in `agent/internal/skills/skills/`.

2. **Entrypoint** — `agent/main.go` (or the file that sets `llmagent.Config.Toolsets`).
