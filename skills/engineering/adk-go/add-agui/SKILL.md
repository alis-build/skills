...
description: >
  ...
  long-running operations (add-lro), or embedded runtime skills (add-agent-skills). No proto or
  define step; service id must match infra config.
...
| 7 | Offer to orient the user in how a request flows (auth → handler → SSE) using `references/request-flow.md`. Recommended when the user is new to AG-UI or asked about auth / exposing to users; skip if they only wanted the wiring |
